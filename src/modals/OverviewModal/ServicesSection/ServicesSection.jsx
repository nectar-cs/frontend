...
class ServicesSectionClass extends React.Component {
  ...
  renderInfoIcon() {
    ...
    return (
      <S.InfoIcon onClick={cb} className="material-icons">
        info
      </S.InfoIcon>
    );
  }
  ...
  renderServiceVisuals() {
    ...
    return services.map((service, i) => (
      <ServiceVisual key={i} index={i} deployment={deployment} service={service} />
    ));
  }
}
  ...
  render() {
    ...
    return (
      <Layout.Div top={2} bottom={7}>
        <Text.H5 top={-2} bottom={2}>
          {title}
        </Text.H5>
        <DepBox deployment={deployment} />
        {this.renderDepSvcArrows()}
        <S.LineTwo>
          <PodBox deployment={deployment} service={service} />
...
class ServiceBoxClass extends React.Component {
  render() {
    ...
    return (
      <S.ServiceBox>
        ...
        <Text.P low={0.2} suck={-0.5}>
          Match pods w/ at least
        </Text.P>
  ...
  renderEndpointsTable() {
    ...
    if (endpoints.length === 0)
      return (
        <Text.P center low={4}>
          {defaults.noEndpoints}
        </Text.P>
      );

    return (
      <Fragment>
        <Text.P low={1.5} suck={-0.5}>
          <b>Endpoints</b> - Actual computed targets
        </Text.P>
        <Tables.SlimTable raw borderless space={0.35}>
          <tbody>{endpoints.map(a => this.renderEndpointRow(a))}</tbody>
        </Tables.SlimTable>
      </Fragment>
    );
  }
  ...
  renderAddressesTable() {
    return (
      <Fragment>
        <Text.P low={1.5} suck={-0.5}>
          <b>Addresses</b> - How to reach me
        </Text.P>
        <Tables.SlimTable raw borderless space={0.35}>
          <tbody>{this.genAddresses().map(a => this.renderAddressRow(a))}</tbody>
        </Tables.SlimTable>
      </Fragment>
    );
  }
    ...
    return (
      <Fragment>
        <Layout.TextLine low={1.5}>
          ...
          <Text.P pushed low={0.2}>
            Means target is a pod in this deployment
          </Text.P>
        </Layout.TextLine>
  ...
  renderAddressRow(addr) {
    ...
    const scopeIcon = scope === 'internal' ? 'settings_input_hdmi' : 'wifi_tethering';
  ...
  renderPodRow(pod) {
    ...
    return (
      <tr key={pod.name}>
        ...
        <td>
          <p>{Utils.tinyPodName(pod.name)}</p>
        </td>
        <td>
          <a href={`http://${pod.ip}`}>
            <p>{pod.ip}</p>
    ...
    return (
      <Fragment>
        <Layout.TextLine low={1.5}>
          <Micon n="check" emotion="success" size="s" />
          <Text.P pushed low={0.2}>
            Means svc '{service.name}' targets this pod
          </Text.P>
        </Layout.TextLine>
        <Layout.TextLine low={0.5}>
          <Micon n="close" emotion="fail" size="s" />
          <Text.P pushed low={0.2}>
            Means it does <b>not</b>
          </Text.P>
        </Layout.TextLine>
      </Fragment>
    );
  ...
  render() {
    ...
    return (
      <S.DepBox>
        ...
        <S.DepBoxHalf>
          <Text.P low={0.2} center suck={-0.5}>
            Create pods w/
          </Text.P>
          <LabelTags labels={templateLabels} />
        </S.DepBoxHalf>
...
const ServicesSection = ModalClientComposer.compose(ServicesSectionClass);

const ServiceBox = ModalHostComposer.compose(ServiceBoxClass);
