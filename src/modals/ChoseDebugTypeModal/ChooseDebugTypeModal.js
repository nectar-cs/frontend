...
export default class ChooseDebugTypeModal extends React.Component {
  ...
  renderRedirect() {
    ...
    return <Redirect to={goToFunc(base, namespace, name)} state={{ deployment, matching }} />;
  }
  ...
  renderComingSoon() {
    ...
    return <ComingSoonSection text={bundle.demo} />;
  }
  ...
  renderActivities() {
    ...
    return (
      <Fragment>
        ...
        <S.ActivitiesContainer>{activities}</S.ActivitiesContainer>
      </Fragment>
    );
  }
...
function ActivityOffering(props) {
  return (
    <S.ActivityContainer onClick={props.callback} isChosen={props.isChosen}>
      <S.ActivityIcon className="material-icons">{props.iconName}</S.ActivityIcon>
