...
export default class HowToAnnotate extends React.Component {
  ...
  renderLines(lines) {
    return lines.map(line => <Text.P>{line}</Text.P>);
  }

  renderCodeExample() {
    const { name, namespace } = this.props.deployment;
    const commands = defaults.howTo.commands(name, namespace);
    const cmdViews = commands.map(command => <Text.Code key={command}>{command}</Text.Code>);

    return <Layout.BigCodeViewer>{cmdViews}</Layout.BigCodeViewer>;
  }
