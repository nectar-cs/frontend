...
export default class BuildPushRunOperation extends BaseOperator {
  ...
  prepareJob(instance) {
    if (instance instanceof TarballJob) this.prepareTarballJob(instance);

    if (instance instanceof DockerBuildJob) this.prepareBuildJob(instance);

    if (instance instanceof DockerPushJob) this.preparePushJob(instance);

    if (instance instanceof ChangeImageTagJob) this.prepareTagChangeJob(instance);

    if (instance instanceof ForceImagePullJob) this.prepareForceReloadJob(instance);

    if (instance instanceof AnnotateDeploymentJob) this.prepareAnnotateJob(instance);
  }
  ...
  jobClasses(bundle) {
    ...
    return [TarballJob, DockerBuildJob, DockerPushJob, PodJob, AnnotateDeploymentJob];
  }
}
