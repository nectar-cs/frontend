...
export default class ImageOpsModal extends React.Component {
  ...
  renderConclusion() {
    ...
    return (
      <Conclusion success={this.state.conclusion} reason={this.opHelper.conclusionMessage()} />
    );
  }
  ...
  renderGamePlan() {
    ...
    if (['docker', 'git'].includes(choices.operationType)) if (!remotes.imageTags) return null;

    return <TermSection title="Game Plan" lines={Helper.previewCommands(this)} />;
  }
  ...
  renderSubmitButton() {
    ...
    return (
      <ModalButton isEnabled={Helper.isInputValid(this)} callback={this.submit} title={'Apply'} />
    );
  }
  ...
  selectedGitCommit() {
    try {
      const { choices, remotes } = this.state;
      const selBranchName = choices.gitBranch;
      return Helper.selectedCommitBundle(remotes, selBranchName, choices.gitCommit);
    } catch {
      return {};
    }
  }
