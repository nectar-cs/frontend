...
export default class PodJob extends Job {
  ...
  newPods() {
    ...
    return this.updated.filter(newPod => !oldNames.includes(newPod.name));
  }

  runningPods(pods = this.updated) {
    return pods.filter(p => p.state.toLowerCase() === 'running');
  }
  ...
  deadPods() {
    ...
    return this.initial.filter(initialPod => !crtRunningNames.includes(initialPod.name));
  }
