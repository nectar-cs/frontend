...
export default class HttpActionsModal extends React.Component {
  ...
  renderGamePlan() {
    const Lines = () =>
      Helper.previewCommands(this).map(cmd => (
        <Text.Code key={cmd} chill>
          {cmd}
        </Text.Code>
      ));
  ...
  renderHistory() {
    if (this.state.showHistory) {
      const historyCallbackSetter = cb => {
  ...
  renderRequestTabs() {
    ...
    const onChange = i => {
      this.setState(s => ({ ...s, showHistory: i === 0 }));
    };

    return (
      <Tabs tabs={REQUEST_TAB_NAMES} selectedInd={0} onTabChanged={onChange}>
        <DestinationPane
  ...
  onSubmitted(response) {
    if (this._isMounted) {
      this.setState(s => ({ ...s, phase: 'response', httpResp: response['data'] }));
