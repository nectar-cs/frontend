...
export default function EventsTimeline(props: TimelineProps) {
  const Items = () =>
    props.events.map((event, i) => <TimelineEvent key={i} {...event} nth={i} mode={props.mode} />);

  const { namespace: ns, name } = props.pod;
  const planCode = defaults.events.gamePlan({ ns, name });

  const PlanLines = () => planCode.map(line => <Text.Code key={line}>{line}</Text.Code>);
  const GamePlan = () => {
  ...
  const EmptyView = () => {
    ...
    return <CenterAnnouncement text="Pod has 0 events." iconName="sentiment_dissatisfied" />;
  };
...
const DateComp = ({ far, str, callback }) => (
  <S.DateContainer far={far} onClick={callback}>
    {str}
  </S.DateContainer>
);
