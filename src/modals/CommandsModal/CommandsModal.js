...
export default class CommandsModal extends React.Component {
  ...
  renderTopLoader() {
    ...
    return <Loader.TopRightSpinner isFetching={true} />;
  }
  ...
  renderHistory() {
    if (!Helper.hasPods(this)) return null;
    const reloadTriggerSetter = v => {
      this.downstreamReloader = v;
    };
  ...
  renderButton() {
    ...
    return (
      <ModalButton title="Execute" callback={() => this.submit()} isEnabled={ready && !executing} />
    );
  }
