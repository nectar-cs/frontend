...
export default class StuntPodRecycleModal extends React.Component {
  ...
  renderExplanation() {
    ...
    const Lines = () => defaults.explanation.lines.map(line => <Text.P low={1.5}>{line}</Text.P>);
  ...
  renderTable() {
    ...
    const PodRows = () => stuntPods.map(pod => <PodRow key={pod.ip} {...pod} />);
...
function PodRow({ namespace, name, updatedAt, state }) {
  ...
  return (
    <tr>
      ...
      <td>
        <Text.BoldStatus emotion={state}>{state}</Text.BoldStatus>
      </td>
      <td>
        <p>{ts}</p>
      </td>
