...
export default class Utils {
  ...
  static labelsToEqStr(labelDict: { [string]: string }) {
    return Object.keys(labelDict)
      .reduce((whole, key) => [...whole, `${key}=${labelDict[key]}`], [])
      .join(',');
  }

  static labelsToDictStrs(labelDict: { [string]: string }) {
    return Object.keys(labelDict).reduce(
      (whole, key) => [...whole, `${key}:${labelDict[key]}`],
      [],
    );
  }
  ...
  static depMatching(deploymentName: string, matchings: Matching[]) {
    return matchings.find(matching => matching.deploymentName === deploymentName);
  }
  ...
  static gitSummary(ms, extended) {
    ...
    const third = extended && ms.dockerBuildPath && <Text.A pushed>{ms.dockerBuildPath}</Text.A>;
  ...
  static senTrack(e) {
    if (typeof e === 'string') Sentry.captureMessage(e);
