...
class WorkspaceShowClass extends React.Component {
  ...
  openRecycleModal() {
    ...
    this.props.openModal(StuntPodRecycleModal, { stuntPods });
  }
  ...
  matchingForDep(deployment) {
    return this.state.matchings.find(matching => matching.deploymentName === deployment.name);
  }
...
const WorkspaceShow = AuthenticatedComponent.compose(
  ModalHostComposer.compose(UpdateCheckComposer.compose(ErrComponent.compose(connected))),
);
