...
export default function WorkspaceDepsPreview(props: Props) {
  ...
  const DeploymentRows = () =>
    deployments.map(deployment => <DeploymentRow key={deployment.name} {...deployment} />);
...
function DeploymentRow(deployment: Deployment) {
  ...
  const strLabels = Object.keys(labels || {}).map(k => `${k}:${(labels || {})[k]}`);

  const LabelViews = () =>
    strLabels.map(label => (
      <p key={label} className={s.statusTag}>
        {label}
      </p>
    ));
