...
function s2P(state) {
  const workspaces = state.mainReducer.workspaces
    .map(w => ({
      id: w.id,
      name: w.name,
      url: `/workspaces/${w.id}`,
...
function SideBarItems(props) {
  return props.items.map(item => (
    <SideBarItem key={item.title} {...item} openModal={props.openModal} />
  ));
}
...
const SideBar = ModalHostComposer.compose(connector(SideBarClass));
