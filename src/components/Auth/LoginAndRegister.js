...
export default class LoginAndRegister extends React.Component {
  ...
  renderSubmitButton() {
    ...
    return (
      <Button.SpicyButton onClick={this.submit} disabled={!isEnabled}>
  ...
  renderAuthenticated() {
    ...
    return <Redirect to="/bulk-matching" />;
  }
  ...
  onAuthFailure(data) {
    ...
    const errors = reasons || ['Unexpected server error :/'];
  ...
  authType() {
    return this.props.match.path.split('/auth/')[1];
  }
